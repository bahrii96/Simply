...
document.addEventListener('wpcf7submit', function (event) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => { $('.wpcf7-form.form-subscribe .error_text').hide(); }, 3000);
    }, 300);
}, false);
    ...
    if (fileInput.files && fileInput.files.length > 0) {
        $.each(fileInput.files, function (key, value) {
            ...
            if (file_type !== 'doc' && file_type !== 'docx' && file_type !== 'pdf' && file_type !== 'xml') {
                ...
                setTimeout(function () { $('.form__field .input-file').removeClass('input-file--uploaded'); }, 100);
...
document.addEventListener( 'wpcf7mailsent', function( event ) {   
    ...
    if ((event.detail.contactFormId == '8447') || (event.detail.contactFormId == '4077') || (event.detail.contactFormId == '10575') || (event.detail.contactFormId == '8444')) { //contacts
        dataLayer.push({'event':'form_contact_us_submission'});
    } else if ((event.detail.contactFormId == '9238') || (event.detail.contactFormId == '9239') || (event.detail.contactFormId == '4593')) { //get_in_touch
        dataLayer.push({'event': 'form_get_in_touch_submission'});
    } else if (event.detail.contactFormId == '4198') { //subscription
        dataLayer.push({'event':'form_blog_subscription_submission'});
    } else if ((event.detail.contactFormId == '14106') || (event.detail.contactFormId == '13946') || (event.detail.contactFormId == '13294')) { //awards_
