...
$category_str = str_replace('/whitepapers/', '', $path);
$category_str = str_replace('/', '', sanitize_text_field($category_str));

if (($category_str == 'whitepapers') or ($category_str == 'ua') or ($category_str == 'pl')) $category_str = '';
...
$exclude_latest = get_posts(array(
...
$exclude_id = $exclude_latest ? $exclude_latest[0] : 0;
...
<?php } else { ?>
	...
	<div class="b-blog-head on-whitepapers">
		<div class="container">
			<div class="title">
			...
			<div class="blog-tags-search">
				<?php if ($categories) { ?>
					<div class="blog-tags">
						<a
							href="<?php echo (pll_current_language() !== 'en') ? '/' . pll_current_language() : ''; ?>/whitepapers/"
							<?php if (!$category_str) echo 'class=" js-tag-filter active" '; ?>>
							<span><?php pll_e('All'); ?></span>
						</a>
						<?php foreach ($categories as $cat) { ?>
							<a
								class="js-tag-filter <?php echo ($cat->slug == $category_str) ? 'active' : ''; ?>"
								href="#"
								data-slug="<?php echo $cat->slug; ?>"
								<?php if ($cat->slug == $category_str) echo 'style="pointer-events: none;"'; ?>>
				...
				<div class="blog-search">
					<form class="search-form" novalidate>
						<label class="with_line">
							...
							<input type="text" name="whitepapers_search" placeholder="<?php echo $pl_search_field; ?>">
							<div class="del">
			...
			<div class="container">
				<div class="cards">
					<?php while ($all_posts->have_posts()) {
						...
						<?php get_template_part('gutenberg/whitepaper_item'); ?>
